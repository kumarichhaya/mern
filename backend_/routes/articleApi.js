...
Article.find({category:cid,publish:true}).sort({date:-1})
.then((data)=>res.send(data))
.catch((err)=>res.send({"Error":"Error in data fetching"}))
})
//by user
...
router.post('/',img.upload.single('image'), (req, res) => {
    ...
    let data={title:b.title,category:b.category,image:image,
        content:b.content,user:b.user
    }
    const rec = new Article(data);
    rec.save()
    .then(()=>{
      let msg=`Article With ${b.title}  Posted By User (${b.user}) at ${Date.now()}</p> `;
      let data={rcvr:"gagandeep.a24@gmail.com",sub:"New Article Entry",body:msg}
